...
debug_set = frozenset(['car00109827', 'car00109824', 'car00109826', 'car00109831', 'car00109843', 'car00109842'])
...
def generate_partition_buffers(partitions, partition_num_ub, partition_size, prune_partition_func):
  partition_buffers = []

  pruned_partitions = [None if prune_partition_func(p) else p \
    for p in partitions]

  for i, p in enumerate(pruned_partitions):
    buffer_start = i - partition_num_ub + 1
    buffer_end = i + partition_num_ub
    # construct.
    if p is not None:
      new_pb = PartitionBuffer(i)
      new_pb.partition = p
      new_pb.partitions = pruned_partitions[max(buffer_start, 0): min(buffer_end, len(pruned_partitions))]
      # print('init p', p.start_fid, \
      #   '[{}]'.format(', '.join([str(x.start_fid) if x is not None else 'None' for x in new_pb.partitions])))
      new_pb.start = partition_size * max(0, buffer_start)
      new_pb.end = partition_size * min(buffer_end, len(partitions)) - 1
      partition_buffers.append(new_pb)

  return partition_buffers

class WorkingPartitionBuffer:
  def __init__(self, partition_buffer: PartitionBuffer, 
    partition_num_lb: int, partition_num_ub: int, 
    partition_size: int, w: int, query: list, labels: set, 
    remainig_max_arr: list, metrics: OnlineMetrics) -> None:
    self.pg = partition_buffer
    ...
    self.labels = labels
    self.query = query
    self.remaining_max_arr = remainig_max_arr
    self.metrics = metrics

    # compute common_mask
    self.sorted_common_objs = partition_buffer.select_common_objs()
    self.common_objs = set(self.sorted_common_objs)

    p = partition_buffer.partition
    ...
    self.remaining_max_arr[self.pg.index] = wp.remaining_max

    self.working_partitions = []
    # working partitions
    for other_p in partition_buffer.partitions:
      # skip empty partitions.
      if other_p is None:
        continue
      if p.start_fid == other_p.start_fid:
        # skip the current partition.
        continue
      partition_mask = self.__compute_mask(other_p.objs)
      mapping = self.__compute_mask_mapping(other_p.objs)
      other_wp = WorkingPartition(other_p, partition_mask, mapping, query, labels)
      mapped_label_masks = self.__compute_mapped_label_masks(other_wp.label_masks, mapping)
      other_wp.mapped_label_masks = mapped_label_masks
      self.working_partitions.append(other_wp)

    self.remaining_max = self.__estimate_max()

    # self.window_score_arr = init_window_score_arr(self.pg.start, self.pg.end, w, 0)

    self.__cache_computed_bitset = set()
    self.min_score = 0
    self.base_idx = partition_buffer.start
  ...
  def __compute_mapped_label_masks(self, label_masks: dict, mapping: dict):
    ...
    for label, mask_and_objs in label_masks.items():
      # convert 
      mapped = map_bitset(mask_and_objs, mapping, len(self.sorted_common_objs))
      mapped_label_masks[label] = mapped
    return mapped_label_masks

  def __estimate_max(self):
    # update remaining max 
    self.remaining_max_arr[self.pg.index] = self.wp.remaining_max
    # return self.wp.remaining_max * self.partition_num_ub
    max_remaining = 0
    for i in range(self.partition_num_ub, len(self.pg.partitions)):
      _pg_start = self.pg.index - self.partition_num_ub + 1 + (i - self.partition_num_ub)
      _remaining = sum(self.remaining_max_arr[_pg_start: _pg_start + self.partition_num_ub])
      if _remaining > max_remaining:
        max_remaining = _remaining
  ...
  def process_until(self, stop_score, bookkeeper):
    ...
    # update max
    self.remaining_max = self.__estimate_max()
    wp:WorkingPartition = self.wp
    while self.remaining_max >= stop_score and self.remaining_max > bookkeeper.min:
      # get next partition
      ...
      if current_node.obj in self.common_objs:
        ...
        if mapped_bitset not in self.__cache_computed_bitset:
  ...
  def __collect_intermediate_results(self, current_node, mapped_bitset, current_node_common_objs, wp):
    ...
    for other_wp in self.working_partitions:
    ...
    for i, partition_results in enumerate(sorted_result_per_partition_list):
      for bitset_info_tuple in partition_results:
        if bitset_info_tuple[0] not in self.__cache_computed_bitset:
          ...
          # if len(bitset_obj_set.intersection(debug_set)) == len(debug_set) and len(bitset_obj_set) == len(debug_set):
          #   print('init current i', i, j)
      ...
      if len(sorted_intervals) > 0:
        ...
        for interval in sorted_intervals:
          for i in range(interval[0], interval[1]+1):
            ...
            #   print('what', interval_start, interval_end, i, 'interval:', interval, 'sorted', sorted_intervals)
            #   print('set', set_and_interval)
            #   raise e
        # sliding widow count.
...
class BitsetMultiLabelProcessor():
  ...
  def __init__(self, partitions, partition_size, total_frames_num, _):
  ...
  def topk(self, query, k, w, pg_size):
    ...
    partition_groups = generate_partition_buffers(self.partitions, 
      partition_num_ub, self.partition_size, __prune_partition)
    
    remaining_max_arr = [0] * len(self.partitions)
    
    # logging.info("number of partition groups: %s", len(partition_groups))
    # map to working partition group
    working_pgs = [WorkingPartitionBuffer(pg, partition_num_lb, 
      partition_num_ub, self.partition_size, w, query, labels, remaining_max_arr, self.metrics) \
